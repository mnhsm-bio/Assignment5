...
from .controllers import orders, sandwiches, resources, recipes, order_details  #each imported from controllers file
...
def update_one_order_detail(detail_id: int, order_detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    detail_db = order_details.read_one(db, detail_id=detail_id)
    if detail_db is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return order_details.update(db=db, order_detail=order_detail, detail_id=detail_id)
